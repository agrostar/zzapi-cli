...
export class Bundle {
  ...
  setBundlePath(relPath: string) {
    ...
    if (!fs.existsSync(this.bundlePath)) throwError(`error: ${this.bundlePath} does not exist`);
    if (!fs.lstatSync(this.bundlePath).isFile()) throwError(`error: ${this.bundlePath} is not a file`);
  }
